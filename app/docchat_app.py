...
class AiQA:
    ...
    @classmethod
    def create_index_faiss(cls, docs, faiss_path):
    ...
    def answer(cls, question):
        if cls.generator is None:
            cls.generator = Seq2SeqGenerator(model_name_or_path="vblagoje/bart_lfqa",
                                use_gpu=True)
            cls.pipe = GenerativeQAPipeline(cls.generator, cls.retriever)
        k_retriever = 3
        cls.res = cls.pipe.run(question, params={"Retriever": {"top_k": k_retriever}})

        print_answers(cls.res, details="minimum")
